...
my $version = "0.4";
my $l2exist = 0;
my $cmd = "Usage: arcstat [-hvxrq] [-f fields] [-o file] [-s string] " .
    "[interval [count]]\n";
my %cur;
my %d;
my $out;
...
sub usage {
        ...
        print STDERR "\t -q : Quient output (no header)\n";
...
sub calculate {
    ...
    if (! defined $d{"evict_skip"}) { $d{"evict_skip"} = 0; }
        $v{"eskip"} = $d{"evict_skip"}/$int;

    if (! defined $d{"recycle_miss"}) { $d{"recycle_miss"} = 0; }
...
sub main {
        ...
        while (1) {
                print_header() if ($i == 0 && ! $quiet_output);
                snap_stats();
                calculate();
                print_values();
